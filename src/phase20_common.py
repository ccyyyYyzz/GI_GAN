...
OUT = E_ROOT / "outputs_phase20_human_written_manuscript"
...
def _runs(values: np.ndarray, threshold: float, min_len: int) -> list[tuple[int, int]]:
...
def grid_cell_bounds(path: Path) -> tuple[list[tuple[int, int]], list[tuple[int, int]]]:
    ...
    arr = np.asarray(im)
    mask = arr < 245
    x_runs = _runs(mask[120:, :].mean(axis=0), 0.05, 120)
    y_runs = [r for r in _runs(mask[:, 20 : arr.shape[1] - 20].mean(axis=1), 0.05, 150) if r[0] > 70]
    if len(x_runs) < 4:
...
def crop_sample(mid: str, row_idx: int) -> dict[str, Image.Image]:
    return {
        ...
        "Backprojection": crop_grid_cell(mid, row_idx, 1),
        "Reconstruction": crop_grid_cell(mid, row_idx, 2),
        "Absolute error": crop_grid_cell(mid, row_idx, 3),
    }
...
def source_manifest() -> dict[str, Any]:
    return {
        ...
        "output": str(OUT),
        "note": "Internal audit manifest only. Main text and submission supplement avoid local Windows paths.",
    }
