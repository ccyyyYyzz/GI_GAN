...
Write-Host "Collecting Phase 51A downloads from $SourceDir"

$zipFiles = Get-ChildItem -LiteralPath $SourceDir -File -Filter "session_0*_outputs.zip" -ErrorAction SilentlyContinue |
...
$downloadManifests = Get-ChildItem -LiteralPath $SourceDir -File -Filter "session_0*_download_manifest.json" -ErrorAction SilentlyContinue |
  Where-Object { $_.Name -match "^session_0[6-9]_" }
...
$splitManifests = Get-ChildItem -LiteralPath $SourceDir -File -Filter "session_0*_split_manifest.json" -ErrorAction SilentlyContinue |
  Where-Object { $_.Name -match "^session_0[6-9]_" }
...
Write-Host "Done. Phase 51A downloads are in $TargetDir"
