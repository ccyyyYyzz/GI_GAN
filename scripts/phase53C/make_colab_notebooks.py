...
OUT = ROOT / "colab" / "phase53C"


SESSIONS = {
    "session_20_exact_null_mi_pretest": {
        "title": "Session 20 exact-null MI/AUC pretest",
        "trains_generator": False,
        "trains_discriminator": True,
        "command": [
            "python",
            "-m",
            "src.phase53C_exact_null_mi_pretest",
            "--bundle_root",
            "{BUNDLE_ROOT}",
            "--output_dir",
            "{SESSION_OUT}",
        ...
        "notes": "Trains critic/classifier only with exact P0 null-space inputs and anchor conditioning.",
    },
    "session_21_soft_leakage_and_shortcut_audit": {
        "title": "Session 21 soft leakage and shortcut audit",
        "trains_generator": False,
        "trains_discriminator": True,
        "command": [
            "python",
            "-m",
            "src.phase53C_soft_leakage_shortcut_audit",
            "--bundle_root",
            "{BUNDLE_ROOT}",
            "--output_dir",
            "{SESSION_OUT}",
        ...
        "notes": "Diagnostic only: tests soft P_N leakage and full-input residual shortcut.",
    },
    "session_22_feasible_hallucination_figure": {
        "title": "Session 22 feasible hallucination figure",
        "trains_generator": False,
        "trains_discriminator": False,
        "command": [
            "python",
            "-m",
            "src.phase53C_feasible_hallucination_figure",
            "--bundle_root",
            "{BUNDLE_ROOT}",
            "--output_dir",
            "{SESSION_OUT}",
    ...
    "session_23_exact_null_critic_evaluator": {
        "title": "Session 23 exact-null critic evaluator",
        "trains_generator": False,
        "trains_discriminator": True,
        "command": [
            "python",
            "-m",
            "src.phase53C_exact_null_critic_evaluator",
            "--bundle_root",
            "{BUNDLE_ROOT}",
            "--output_dir",
            "{SESSION_OUT}",
        ...
        "notes": "Trains an exact-null critic as evaluator, not as generator loss.",
    },
    "session_24_optional_gan_and_posterior_sampling": {
        "title": "Session 24 optional GAN and posterior sampling",
        "trains_generator": True,
        "trains_discriminator": True,
        "command": [
            "python",
            "-m",
            "src.phase53C_optional_gan_posterior",
            "--bundle_root",
            "{BUNDLE_ROOT}",
            "--output_dir",
            "{SESSION_OUT}",
        ...
        "notes": "GAN is gated by exact-null AUC; otherwise only posterior diagnostics run.",
    },
}
...
OUT_ROOT = Path("/content/outputs_phase53C_exact_null_critic")
...
PROJECT_ZIP_NAME = "ns_mc_gan_gi_project_phase53C.zip"
BUNDLE_ZIP_CANDIDATES = ["noleak_bundle_phase53C.zip", "noleak_bundle_phase48_49.zip"]
DRIVE_UPLOAD_DIR = Path("/content/drive/MyDrive/ns_mc_gan_gi/colab_upload")
DRIVE_OUTPUT_DIR = Path("/content/drive/MyDrive/ns_mc_gan_gi/outputs_phase53C_exact_null_critic")
...
def unzip_project(project_zip):
    tmp = Path("/content/_phase53C_project_unzip")
...
def make_notebook(session_name: str, meta: dict) -> dict:
    cells = [
        {"cell_type": "markdown", "metadata": {}, "source": [f"# Phase53C - {meta['title']}\n", "\n", f"{meta['notes']}\n"]},
        {
            "cell_type": "code",
