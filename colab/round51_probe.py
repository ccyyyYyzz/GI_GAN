...
processes = subprocess.run(
    ["bash", "-lc", "ps -eo pid,etimes,cmd | grep -E 'run_operator_seed_campaign|diagnose_fiber_orthogonal' | grep -v grep || true"],
    check=True,
    capture_output=True,
    text=True,
).stdout.strip()
...
root = Path("/content/gan_r51_results")
for result_dir in sorted(root.glob("operator_seed_*")):
    ...
    for name in ("driver.log", "cache.log", "gan_adv0.log", "control.log", "fohi.log"):
        log = result_dir / name
        if log.is_file():
            print("LOG_TAIL", name)
