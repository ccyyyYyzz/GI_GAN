...
def main(lam=1e-3):
    ...
    axb.axvline(1.0, color=C_RED, lw=2.2, label="flat-spectrum op. ($\\sigma\\equiv1$)")
    ...
    # MRI single point
    axc.plot([1.0], [lam / (lam + 1.0)], "s", color=C_RED, ms=9, zorder=6, label="flat-spectrum operator")
    ...
    fig.suptitle("A non-uniform singular spectrum makes the per-mode contraction visible "
                 "(a flat-spectrum operator is a single point)", fontsize=11.5, y=1.02)
    for ext in ("png", "pdf"):
        fig.savefig(os.path.join(OUT, "CASSI_SPECTRUM_OPTICS." + ext), dpi=170, bbox_inches="tight")
