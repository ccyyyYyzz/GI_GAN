...
In undersampled ghost imaging, the recorded buckets $y = Ax + \varepsilon$ with $m \ll n$ fix only the row-space component of the scene: since $A P_0 = 0$ for the null-space projector $P_0 = I - A^\dagger A$, the content carrying most perceptual detail is exactly invisible to the measurement. We take this identity seriously in three steps. First, a constructive converse: for any target we build a feasible-but-wrong witness — a semantically different scene matching the target's record to $\sim\!2\times10^{-15}$, tighter than the noisy truth's own residual ($\sim\!10^{-3}$ at bucket noise $\sigma_\varepsilon = 0.01$) — so measurement consistency can never certify null-space content. Second, an achievability: a ground-truth-free test-time audit that contracts each measured mode by exactly $\lambda/(\lambda+\sigma_i^2)$, driving measurement residuals down by orders of magnitude at negligible quality cost and separating accountability from quality. Third, governed use: a pre-registered scalar dial fuses the null-space contributions of a reconstruction prior and an adversarial prior with $A\hat{x}_B = y$ exact at every setting, improving locked LPIPS by 32.6% within a bounded distortion budget. A no-adaptation lemma shows ground-truth-free per-image adaptation is impossible on a fixed record, and projector forensics of three published pipelines, on their own operators, find the same two-ledger structure: certifiable gains saturate, and headlines are paid from the null space. Measurement consistency certifies reproduction of the record — never the truth of invented detail.
...
**Converse (the barrier, §3).** Because $y$ fixes exactly $P_R x$ and nothing else, null-space content is *provably* unverifiable from the measurement. We make this constructive rather than abstract: for any two images $x_i, x_j$ the spliced image $u_{ij} = x_j - A^\dagger(A x_j - y_i)$ carries the measured component of $x_i$ but the null-space content of $x_j$, and satisfies the record of $x_i$ to floating-point precision. Empirically these witnesses match the record at floating-point round-off — tighter than the noisy truth matches its own record — while remaining semantically wrong (all constructed pairs; numbers in §3.2). Consistency is not correctness.

**Achievability (the certificate, §4).** The same geometry that forbids certifying $P_0 x$ *does* permit a ground-truth-free, test-time audit of the measured component. Attaching $\Pi_y^\lambda(v) = v - A^\top(A A^\top + \lambda I)^{-1}(A v - y)$ to any reconstructor contracts each measured singular mode by exactly $\lambda/(\lambda + \sigma_i^2)$ and leaves $P_0 v$ untouched. The audit reduces measurement residuals by orders of magnitude across analytic, variational, and learned reconstructors at negligible PSNR cost, cleanly separating image quality from measurement accountability, and matches the closed-form contraction at the float64 floor (§4).
...
gives $A \hat{x}_B = y$ *exactly* for every $B$. At the balanced operating point this improves locked LPIPS by 32.6% at a bounded PSNR cost, passing an 8/8 pre-registered acceptance gate on a raw-hash-disjoint locked split (§7).

**How to read this paper.** Figure 1 is the map. One identity ($A P_0 = 0$, §2) forks into a *cannot* (the constructive converse, §3) and a *can* (the exact certificate, §4–5); together they license the *therefore* (the governed dial, §6–7); and the two closing branches test the frame against its own limits (§7.4) and against the published field (§8.4). Every section opens by naming its place on this map, and a reader who keeps Figure 1 in view will never be more than one arrow away from the main line.

[FIG: ROADMAP.pdf | 1.0 | **The map of the argument.** One identity forks into a converse and an achievability; together they license governed injection; the frame is then tested against its own limits and against three published pipelines. Section numbers give the route.]

### 1.3 Overview figure

Figure 2 renders this chain as three panels — *cannot / can / therefore*. The left panel (**cannot**) shows a feasible-but-wrong pair: a true target and its spliced counterpart, sharing the measured row space and differing only in the null space, whose bucket residual ($2.93\times10^{-15}$) is smaller than the truth's own noisy residual ($5.36\times10^{-3}$) — a measurement-consistent, semantically wrong, non-natural image. The middle panel (**can**) shows the exact per-mode certificate: the audit contracting each measured singular mode by $\lambda/(\lambda+\sigma_i^2)$, driving residuals to the noise floor while PSNR barely moves. The right panel (**therefore**) shows metered injection at $B = 0.55$, with the fused reconstruction stamped $A\hat{x} = y$ — invented detail placed only where the bucket is blind, without ever breaking the measurement.

[FIG: METHOD_FIG1.pdf | 1.0 | **the deductive chain as three panels.** *Cannot:* a feasible-but-wrong witness reproduces the target's bucket record to $2.93\times10^{-15}$ relative error — tighter than the noisy truth's own $5.36\times10^{-3}$ — while depicting a different scene; null-space content is unverifiable from the record. *Can:* the ground-truth-free audit $\Pi_y^\lambda$ contracts each measured singular mode by exactly $\lambda/(\lambda+\sigma_i^2)$, driving the measurement residual by orders of magnitude at negligible PSNR cost. *Therefore:* prior detail is injected only inside the null space through the dial $B$, and $A\hat{x}_B = y$ holds exactly at every setting.]
...
4. **A no-adaptation lemma, with its empirical sharpness.** Any selection rule computable from one record is constant on each measurement fiber (an elementary corollary of the converse we state formally in §7.4), so ground-truth-free per-image adaptation reduces to the $y$-predictable component of the oracle — which, on this benchmark [development-level], is worth $\le 0.002$ LPIPS beyond a matched-PSNR constant (feature–oracle correlations $|\rho|\le0.24$). The honest dial is a scalar: exactly so across each fiber, empirically so on this dataset.
5. **Projector forensics of published pipelines.** We decompose the reported gains of three published DL-GI methods — pretrained+fine-tuned (Wang et al. 2022a), untrained DIP on real measured speckle (GIDC), and self-supervised on a noisy record (Noise2Ghost) — on **their own operators with their own code**, and find one law: the certifiable row ledger saturates (terminal error $93$–$97\%$ null) and every headline PSNR stands $+3.1$ to $+8.1$ dB above its own operator's range ceiling, paid from the null space (§8.4). The separation law describes the field, not just our construction.
...
*On the map (Fig. 1): the* cannot *branch.*
...
*On the map: the* can *branch — what the same geometry still permits.*
...
[FIG: CERT_SEPARATION.pdf | 1.0 | **The certificate and the separation.** *(a)* Each measured mode's residual contracts by the closed-form factor $\lambda/(\lambda+\sigma_i^2)$ — smaller $\lambda$, harder projection. *(b)* Applied post-hoc to four unrelated reconstructors, the audit moves measurement accountability by orders of magnitude while PSNR barely moves (green): the two axes are decoupled exactly because $AP_0 = 0$.]
...
*On the map: still the* can *branch — what the certificate does and does not buy.*
...
*On the map: the* therefore *— the converse licenses injection exactly where the certificate cannot reach.*
...
This is the constructive counterpart of the verifiability barrier. In §3 the identity $A P_0 = 0$ produced a feasible-but-wrong witness that satisfied the record to $\sim\!2\times10^{-15}$ — content that is provably unverifiable. Here the *same* identity guarantees that no matter how much adversarial detail the dial injects, the reconstruction never leaves the feasible set. The audit and the fusion are two consequences of one fact: what the bucket cannot certify, editing cannot corrupt. Figure 5 draws the mechanism.

[FIG: METHOD_DIAGRAM_3D.pdf | 0.92 | **the mechanism.** Every measurement-consistent image lies on the affine flat $\{x: Ax=y\} = x_0 + \mathcal{N}(A)$; the bucket fixes only the orthogonal row-space direction, so leaving the flat changes $y$. From the measurement-audited LMMSE anchor $x_0$, the fusion weight $B$ slides the reconstruction along the in-plane dial from the VQAE structure point ($B{=}0$) to the VQGAN detail point ($B{=}1$). Because $AP_0 = 0$, every $B$ stays on the flat and $A\hat{x}_B = y$ holds exactly. The true scene $x^\ast$ also lies on the flat, but its null-space location is unknowable from $y$ — the same geometry read as barrier and as license.]

### 6.4 The identity holds numerically

The theorem is exact in exact arithmetic; in floating point the residual is at round-off. Across the locked split (5.0% sampling, $64\times64$ grayscale STL10, $n = 4096$, $m = 205$, 3 seeds), the relative measurement error $\mathrm{RelMeasErr} = \lVert A\hat{x}_B - y\rVert / \lVert y \rVert$ has mean $3.6\times10^{-7}$ and max $5.7\times10^{-7}$ — the float32 pipeline's round-off scale (the float64 witnesses of §3 sit at $10^{-15}$), identical for every operating point on the dial, confirming that raising $B$ buys perceptual detail at no measurement cost. This is the achievability that the impossibility result licenses: a supply of null-space detail that is, by construction and in measurement, free of any consistency penalty.
...
*On the map: still the* therefore *— choosing, locking, and stress-testing one operating point.*
...
[FIG: PARETO_FIGURE.pdf | 0.9 | **the metered perception–distortion ladder.** Named operating points of Table 1 on the perception–distortion plane (LPIPS versus PSNR/RMSE): the LMMSE anchor, the VQAE structure branch ($B{=}0$), the pre-registered balanced point ($B{\approx}0.55$), quality-lite ($B{\approx}0.72$), and full VQGAN ($B{=}1$). The ladder is monotone — perceptual quality is bought at a metered pixel-fidelity cost — and every point satisfies $A\hat{x}=y$ to $\mathrm{RelMeasErr}\sim10^{-7}$; the trade moves entirely within the null space.]

[FIG: B_CURVE.pdf | 0.85 | **the frontier is smooth, monotone, and interior-optimum-free.** A dense 21-point development-split sweep of the fusion weight $B$ (step $0.05$): LPIPS falls strictly monotonically from $0.293$ to $0.167$ while PSNR declines from $22.87$ to $21.17$ dB (one shallow interior maximum of $+0.03$ dB at $B=0.10$). No discontinuity and no interior perceptual optimum — the dial is a stable interpolation, not a fragile hyperparameter.]
...
[FIG: QUALITATIVE_GRID.pdf | 1.0 | **locked-split reconstructions at the named operating points.** Fixed samples reconstructed by the LMMSE anchor, the VQAE branch, balanced fusion, quality-lite, and full VQGAN, against ground truth. Moving right along the dial adds perceptual detail — sharper texture, more natural spectra — while every column reproduces the identical bucket record exactly; the added detail lives entirely in the null space the measurement cannot see.]
...
*On the map: the outer branches — how far the locked point travels, and what the frame says about the published field.*
...
[FIG: rate_generalization_figure.pdf | 0.85 | **the perceptual gain generalizes across sampling rate (development-level).** Balanced fusion's relative LPIPS gain over the VQAE branch at $2\%$, $5\%$ (locked), and $10\%$ sampling, with the PSNR cost within the pre-registered $-0.5$ dB validation tolerance at every rate (3 seeds per rate, all same-direction).]
...
[FIG: NOISE_ROBUSTNESS.pdf | 0.85 | **the recommended dial position is also the robust one (development-level).** LPIPS versus bucket-noise level for the VQAE branch, balanced fusion, and full VQGAN with $B$ frozen: full VQGAN's synthesized fine detail amplifies measurement noise and degrades sharply, while balanced fusion overtakes it at $\sigma_\varepsilon = 0.02$ and degrades gracefully; the exact consistency $A\hat{x}_B = y$ concerns the row space at every noise level and is unaffected.]
...
[FIG: FORENSICS_CROSS_TARGET.pdf | 1.0 | **projector forensics of three published DL-GI pipelines, on their own operators.** *(a)* The fine-tuning trajectory of the physics-enhanced pipeline (Wang et al. 2022a), decomposed per step: PSNR rises past the operator's range ceiling (dashed) — everything above it is paid from the null ledger — while the null share of the remaining error climbs to $96.9\%$. *(b)* Attribution of each method's MSE improvement: row-space (certifiable) repair versus null-space (uncertifiable) injection — $61/39$ for fine-tuning, $60/40$ for GIDC's untrained DIP on real measured speckle, $5/95$ for Noise2Ghost against least squares on a noisy record. *(c)* Every headline PSNR stands $+3.1$ to $+8.1$ dB above its own operator's range ceiling, with terminal error $93$–$97\%$ null.]
...
The certificate of §4 and the dial of §6 are one fact read in two directions. Because $A P_0 = 0$, the record fixes exactly $P_R x$. Read forward, the identity is the guarantee: the audit certifies agreement with the record on the measured modes, ground-truth-free, and leaves the null space untouched ($P_0\,\Pi_y^\lambda(v) = P_0 v$). Read backward, it is the limit: no test on $y$ can distinguish among images sharing $P_R x$ — the converse of §3. The property that lets the audit certify the row space is the same property that makes the null space uncertifiable, which is why the two guarantees can never be summed into one.

### 9.1 A prior can raise naturalness without reducing distance to the true null content

Take any two audited-and-fused reconstructions sharing an operator and anchor. Both satisfy $A\hat{x} = y$, so both have the identical row-space component, and their entire difference lives in the null space:
$$
\hat{x}_{B_1} - \hat{x}_{B_2} = P_0\big[(B_1 - B_2)(d_G - d_A)\big] \in \mathcal{N}(A).
$$
Whatever a stronger prior buys — the locked perceptual and distributional gains of §7 — it buys entirely in $P_0$. The null-space error, however, is $P_0(\hat{x} - x) = P_0\hat{x} - P_0 x$, and its second term is the true scene's unmeasured content, the one quantity the record cannot reveal. A prior that increases naturalness changes $P_0\hat{x}$; it carries **no guarantee** of reducing the distance to $P_0 x$ — a more realistic reconstruction may be nearer to, or farther from, the true null content than a flatter one, and the measurement provides no way to tell which (the prior-content map $|P_0\hat{x}|$ of §3.4 exhibits the same decoupling directly). Perceptual gain is real and is measured on the quality axis; it must never be read as evidence on the null axis, because $A P_0 = 0$ severs the two.

### 9.2 Prescription: report both axes, and label provenance

The discipline that follows is procedural, and we adopt it throughout. First, **report measurement accountability and image quality as separate axes**, never one as a proxy for the other: RelMeasErr certifies agreement with the record; LPIPS, KID, and RAPSD describe perceptual quality; PSNR and RMSE describe pixel distortion. A high value on either axis licenses no inference about the other (§5). Second, **label every reconstructed coordinate by provenance**: the $m$ row-space coordinates are *measured* — pinned by $y$ and certifiable by the audit; the null-space coordinates carrying injected detail are *prior-supplied* — a modeling choice, exact against the record by $A P_0 = 0$, and openly attributed to the prior; and the true null content $P_0 x$ is *unverifiable* — no test on $y$ can certify it. These three labels — **measured / prior-supplied / unverifiable** — are the whole synthesis, and the boundary is stated without exception: $A\hat{x} = y$ certifies that a reconstruction reproduces the buckets; it never certifies that the injected texture is the true scene.
...
Everything beyond that one point is **development-level** and is reported as such. The cross-rate study (a $-0.116$/$29.3\%$ LPIPS gain at $2\%$, $-0.098$/$32.6\%$ at $5\%$, $-0.076$/$34.2\%$ at $10\%$, $3/3$ seeds each) and the measurement-noise sweep (balanced fusion overtaking full VQGAN at $\sigma=0.02$, $0.197$ vs. $0.204$, and beating it at $\sigma=0.05$, $0.250$ vs. $0.293$) are exploratory: $B$ is selected on validation under the same tolerance rule, no gate is fixed in advance, and no confirmatory claim is made (the cross-rate runs score on the development split; the noise sweep of §8.2 re-reads the locked images post-gate, with the dial frozen and nothing selected, as disclosed in §7.2). We have not locked any rate other than $5\%$, not locked the noisy-acquisition result, and not tested other operator designs or resolutions. The result does **not** establish that every low-rate task benefits.

### 11.3 The GAN branch is a representative prior, not a state-of-the-art claim

The detail branch uses a VQGAN because its adversarial patch objective supplies exactly the kind of unverifiable high-frequency null-space content the framework is built to govern. It is a controlled, representative instance of an adversarial prior under a measurement certificate — not a claim that this VQGAN, or GANs generally, are the best generative model for the problem. We do not compete on restoration quality and do not claim to beat diffusion. A paired-seed audit in a companion case study (manuscript in preparation) points the same way: the adversarial branch beats its matched control consistently but by a margin of hundredths of a decibel, not a dominance result. The adversarial prior earns its place as a *governable source of detail*, not as a performance headline.
