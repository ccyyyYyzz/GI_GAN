...
In low-rate ghost imaging, measurement consistency is not evidence of image correctness. The reason is geometric: the record constrains only the measurement-visible row-space coordinate — $P_R x = A^\dagger y$ in the noiseless model, its noisy counterpart $A^\dagger y = P_R x + A^\dagger\varepsilon$ otherwise — while $P_0 x \in \ker A$ is invisible to the bucket. We turn this fact into an accountability framework. First, we construct feasible-but-wrong witnesses that match a target record to floating-point precision, around $10^{-15}$ — more tightly than the noisy truth matches its own record, around $10^{-3}$ — while remaining semantically far from the target. Second, we give a test-time row-space audit: applied after any reconstructor, it contracts the $i$-th measured residual mode by exactly $\lambda/(\lambda+\sigma_i^2)$, certifying agreement with the recorded bucket without ground truth. Third, we use the same boundary as a governance rule: prior detail is injected only through $P_0$, so $A\hat{x}_B = y$ holds for every dial setting $B$. We validate the framework on locked $64\times64$ STL10 at 5% sampling: the governed dial reduces LPIPS by 32.6% relative to the projected VQAE endpoint at a 0.45 dB PSNR cost while preserving exact record consistency. A projector audit of three published DL-GI pipelines shows the same ledger structure on external operators. The result is a two-ledger account: certify what the bucket measured; label and govern what the prior supplied.
...
2. **Exact record-consistency audit.** A post-hoc audit whose residual in each measured singular mode contracts by exactly $\lambda/(\lambda+\sigma_i^2)$, certifying agreement with the recorded bucket independently of the reconstructor (§3).
...
|---|---|---|
| Null content is not certifiable from one record | theorem + witness | Proposition 1; witness residuals (§2) |
| Record consistency is certifiable, mode by mode | theorem | modal contraction (§3) |
| Prior detail can be supplied without breaking record consistency | theorem | $A\hat{x}_B = y$ (§4) |
| Balanced dial improves perception at bounded distortion | locked experiment | 32.6% LPIPS, 8/8 gate (§4.3) |
| Per-image GT-free dial adaptation adds little | development-level | $\le 0.002$ LPIPS (§4.5) |
| Published DL-GI gains show the same ledger structure | external audit | 3 pipelines (§5) |

The experiments use signed, gray-valued computational GI operators (row-orthonormalized for the dial's operator) on $64\times64$ grayscale STL10 — deterministic basis-scan single-pixel operators, implementable in principle by complementary exposures with multi-level modulation as in Fourier single-pixel imaging. We do not model DMD quantization, photon budgets, or Poisson statistics; all quantitative claims are simulation-level (§7).
...
The factor itself is classical — the complement of the Tikhonov filter factor (Engl, Hanke & Neubauer 1996; Hansen 1998), and MacKay's (1992) effective-parameter summand. The contribution is its use: a post-hoc, per-record audit after arbitrary reconstructors, paired with the converse of §2 (positioning: §6, Appendix D).
...
**Hard limitations.** The study is simulation-only, at one resolution, with locked claims on one 5% operator; there is no photon-level noise model — bucket noise is Gaussian, a first-order proxy for the signal-dependent Poisson statistics of photon counting — and no hardware validation. The true null content remains uncertifiable by any within-record method — that is the theorem, not an engineering gap.
...
Low-rate ghost imaging has a two-ledger structure. The row ledger is measurement-visible and can be audited exactly against the recorded buckets. The null ledger carries most perceptual detail, and the record supplies no evidence about its truth.

We made both sides constructive: feasible-but-wrong witnesses — including box-constrained, $[0,1]$-legal ones — expose the barrier; the modal audit certifies the measured record; and the governed dial supplies prior detail only through $P_0$, preserving $A\hat{x}_B = y$ at every setting while exposing the perception–distortion trade. On external operators, three published pipelines show the same ledger structure.
...
- **Certifiable (the $m$ row coordinates).** The component $P_R x = A^\dagger y$ is fixed by the record: any measurement-consistent reconstruction must have $P_R \hat{x} = A^\dagger y$, and re-measuring it reproduces $y$. These $m$ coordinates are pinned down and can be audited without ground truth (main §3).
