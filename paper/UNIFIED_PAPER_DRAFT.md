# Certify What You Measure, Govern What You Cannot: A Range–Null Account of Accountability and Governed Detail Across Undersampled Imaging Operators
...
**Proposition 1 (feasible-but-wrong splice).** *Assume $A$ has full row rank. For any record $y \in \mathbb{R}^m$ and any donor image $x_j$, define*
...
*The donor's null content is imported wholesale while the target record is preserved exactly.* (Proof: $AA^\dagger = I_m$ and $P_0 A^\dagger = 0$.)
...
### 2.2 A wrong image matches the noisy record more tightly than the clean truth does
...
## 3. The Record-Consistency Audit
...
The dial is not a fusion architecture. It is a provenance constraint: whatever prior is used, its contribution is projected into $\ker A$, so perceptual detail is supplied without borrowing measurement authority. We call the control *governed* because it changes only what the measurement cannot certify and reports that change as prior-supplied detail.
...
**Theorem 2 (exact consistency for every $B$).** $A\hat{x}_B = A x_0 + A P_0(\cdot) = y$ *for all* $B \in \mathbb{R}$. The identity that makes null content unverifiable (§2) is the identity that makes injecting it measurement-safe. Numerically the locked splits hold $\mathrm{RelMeasErr}$ at $3.6\times10^{-7}$ (float32 pipeline scale) for every operating point.

*Pixel-range protocol.* The identity holds for the unclipped reconstruction. About $3\%$ of output pixels fall outside $[0,1]$; all reported residuals are computed before clipping, quality metrics (LPIPS/PSNR/SSIM) are computed on the clipped image, and figure panels are clipped only for rendering — the clipped display object satisfies the record to ${\sim}4\times10^{-3}$, and clipping is not part of the reconstruction pipeline.

[FIG: DIAL_MECHANISM.pdf | 0.8 | The dial moves only inside the measurement fiber. Since $AP_0 = 0$, changing $B$ changes prior-supplied null content without changing the recorded bucket.]
...
**Table 1. Locked-split means (512 images, 3 seeds).** All VQAE/VQGAN endpoints are row-space anchored, $x_0 + P_0(x_\cdot - x_0)$, not raw network outputs.
...
**Lemma 2 (no-adaptation).** *(i) With a noiseless record, any selection rule $g(A, y)$ is constant on each fiber $\{x: Ax = y\}$: no such rule can distinguish fiber-mates. (ii) In the general model, for $B^\star \in L^2$ and squared loss on the weight, the best record-measurable selector is the conditional mean*
...
*so the irreducible adaptation error is the oracle's conditional variance given the record.* (Proof: (i) fiber-mates share $y$, hence share $g$; (ii) is the conditional-expectation projection theorem. $\blacksquare$) Squared loss on the weight is an analysis device; the evaluation below scores LPIPS regret directly.
...
[FIG: FORENSICS_CROSS_TARGET.pdf | 1.0 | **Three paradigms, three operators, one structure.** *(a)* A fine-tuning trajectory decomposed per step: PSNR rises past the operator's range ceiling — everything above it is paid from the null ledger. *(b)* Attribution of each method's improvement: row repair vs null supply. *(c)* Every headline stands $+3.1$ to $+8.1$ dB above its own range ceiling with terminal error $93$–$97\%$ null.]
...
**The witness transfers.** The §2 construction — a feasible-but-wrong record twin — reappears unchanged. Two *different* knees are forced to share one identical $R{=}8$ (nominal; effective $6.9\times$ after the auto-calibration region) undersampled record: the two records match to median $2.9\times10^{-16}$, and a magnitude-legal, $[0,1]$-projected POCS witness matches to $2.5\times10^{-16}$. The witness is a visibly different knee at $20.3$ dB PSNR to the target, yet is measurement-indistinguishable from it. As in §2, consistency is not correctness; the geometry is the same, only the operator changed.

**Forensics on the official pipeline.** We run the §5 ledger against the released fastMRI single-coil U-Net (chans $=256$, wired from the shipped state dict). Here the accounting reference is exact, not estimated: the zero-filled reconstruction *equals* $P_R x_\text{gt}$, so its magnitude PSNR *is* the range ceiling. At $R{=}4$ (eff. $3.7\times$) the ceiling is $26.70$ dB and the U-Net reaches $29.68$ dB, a $+2.98$ dB gain that is $91\%$ null-supplied; at $R{=}8$ (eff. $6.9\times$) the ceiling is $24.32$ dB, the U-Net $25.42$ dB, gain $+1.09$ dB, $92\%$ null. As in the GI forensics of §5, the headline sits *above* the operator's range ceiling and the surplus is paid from the null ledger — but here the U-Net also drifts off the record it was given ($5.9\%$ at $R{=}4$, $3.3\%$ at $R{=}8$), which undersampling does not entail. A *governed* data-consistent variant $A^\dagger y + P_0 x_\text{UNet}$ — the null-space dial of §4 applied post-hoc — keeps almost all of the gain ($29.42$ vs $29.68$ dB at $R{=}4$) at *exact* record consistency (drift $3.5\times10^{-16}$). The GT-free audit of §3 then contracts the U-Net's record drift by exactly $\lambda/(\lambda+1)$ — the flat-spectrum collapse of the §3.2 factor — from median $5.86\times10^{-2}$ to $5.85\times10^{-5}$ at $\lambda=10^{-3}$, per-mode deviation $\le10^{-15}$.

**A governed certified decision.** On $429$ real fastMRI+ pathology bounding boxes we push the framework to a decision. The auto-calibration region certifies the *coarse* lesion — full-box null-energy median $13.5\%$ at $R{=}8$, $18.4\%$ at $R{=}16$ — but the *fine detail* inside the box is $88.4\%$ ($R{=}8$) / $93.8\%$ ($R{=}16$) null-supplied. A governed null-space edit therefore changes a soft-tissue lesion's fine-detail contrast by $-43\%$ while $Ax=y$ holds to $3.6\times10^{-16}$. We state the honest scope plainly: at clinical $4$–$8\times$ the calibration region retains the diagnostic low-frequency content, so there is no "erase the lesion" — the certifiable claim is *coarse-certified, fine-detail-uncertified*, not lesion deletion (full decomposition and the decision figure: Appendix E).

### 6.2 CASSI on the real released coded aperture

Single-disperser CASSI shifts, masks and sums a $28$-band cube into a $256\times310$ snapshot — a $23.1\times$ undersampling — through the shift-mask-sum operator $A$. We build $A$ from the *real released* TSA-Net gray coded aperture (fill $0.563$). This operator has a rare exact structure: $AA^\top = \operatorname{diag}(\Phi_s)$ is diagonal, so $A^\dagger$, $P_R$, $P_0$ and the entire singular spectrum are closed-form, $O(N)$, with no SVD. The projector identities hold to ${\sim}10^{-13}$ in float64.

**The non-uniform spectrum: §3.2 made visible.** The singular values $\sigma_j=\sqrt{\Phi_s}$ over $79{,}360$ measured detector modes span $[0.009,\,4.907]$ (median $3.51$) — a $528\times$ ratio. The per-mode audit contraction $\lambda/(\lambda+\sigma_j^2)$ at $\lambda=10^{-3}$ therefore spans $[4.15\times10^{-5},\,9.21\times10^{-1}]$, a $2\times10^{4}$ spread — precisely the modal-contraction theorem of §3.2, where masked-Fourier MRI (§6.1) collapses to the single value $9.99\times10^{-4}$. This is the empirical face of the closed form: the same audit that is one scalar under a flat spectrum resolves into a full graded profile under a non-uniform one.

[FIG: CASSI_SPECTRUM.pdf | 1.0 | **The modal-contraction theorem, made visible.** The CASSI singular spectrum $\sigma_j=\sqrt{\Phi_s}$ over $79{,}360$ detector modes spans a $528\times$ range; the per-mode audit factor $\lambda/(\lambda+\sigma_j^2)$ of §3.2 spreads across four orders of magnitude, where flat-spectrum masked-Fourier MRI (§6.1) collapses to a single point (dashed).]

**The witness transfers.** Two *different* spectral scenes are forced to share one identical $23\times$ coded snapshot: matched to median $1.7\times10^{-16}$, with a box-legal ($[0,1]$) POCS witness to $1.9\times10^{-3}$. The witness carries the donor's null-space spectral content, so the two scenes' per-pixel spectra differ visibly under an identical snapshot — the §2 impossibility, now in the spectral channel.

[FIG: CASSI_WITNESS.pdf | 1.0 | **Two spectra, one snapshot.** Two spectrally distinct scenes matched to an identical $23\times$ CASSI snapshot (median $1.7\times10^{-16}$). Per-pixel spectra (insets) differ visibly, yet the coded records are indistinguishable: measurement consistency does not fix the spectral cube.]

**Forensics over thirteen published models.** We run the §5 ledger across $13$ published reconstruction models from the MST model zoo on the shared operator, with the wiring validated by reproducing each model's published simulation PSNR (MST-S $34.38$ vs $34.26$; HDNet $35.06$ vs $34.97$; BIRNAT $37.73$ vs $37.58$; DAUHST-9stg $38.18$ vs $38.36$). The min-norm $A^\dagger y$ range ceiling is $19.04$ dB, and *twelve of the thirteen* models supply $100$–$101\%$ of their PSNR gain over that ceiling from the null space, with the row-effect slightly *negative* — the models mildly corrupt the measured component rather than repairing it; GAP-Net is the sole exception ($93\%$ null, a positive row-effect, and the largest record drift). The entire decade of progress, TSA-Net $31.67 \to$ DAUHST-9stg $38.18$ ($+6.5$ dB), happens wholly in the null ledger, and the models are $0.9$–$12\%$ record-inconsistent. This mirrors the GI ledger of §5 in a modality with $23\times$ more undersampling and $13\times$ more pipelines.

[FIG: CASSI_FORENSICS.pdf | 1.0 | **A decade of SOTA in the null ledger.** Thirteen published CASSI models decomposed on their shared operator against the $19.04$ dB min-norm range ceiling. Twelve of the thirteen gains (TSA-Net $\to$ DAUHST-9stg, $+6.5$ dB) are $100$–$101\%$ null-supplied (GAP-Net the exception at $93\%$); the row-effect is slightly negative (models move the measured component *away* from $A^\dagger y$) and each is $0.9$–$12\%$ record-inconsistent.]

### 6.3 What the two operators establish together

**The bracket.** MRI and CASSI bracket the geometry of §3.2 from both ends. MRI is flat and well-conditioned ($\sigma_i\equiv1$), clinically familiar, and its modal contraction is a single scalar; CASSI is extreme and non-uniform ($528\times$), and its contraction is a four-order-of-magnitude profile. The witness, the projector identities, the GT-free audit, and the forensic ledger hold at both ends and everywhere the operator can sit between them. Together with the GI operator of §5, the account now spans single-pixel optics, Fourier imaging, and dispersive spectral imaging on operators drawn from three real measurement systems.

**Tautology defense.** As §5's caveat box already concedes, part of the direction is forced at $m\ll n$, and we keep definitional and contingent apart. That a record-consistent estimator's error is null-dominated is *not* a finding: $AP_0=0$ and the orthogonal split $\|e\|^2=\|P_R e\|^2+\|P_0 e\|^2$ make it true by construction whenever most degrees of freedom are unsampled, and the min-norm reference has a $100\%$-null terminal error by definition. What is *contingent* — not entailed by $m\ll n$, and therefore the actual content — is fourfold: (i) the measured record-*inconsistency* of the published models ($5.9$/$3.3\%$ for the U-Net, $0.9$–$12\%$ across the CASSI zoo), which a data-consistent estimator would not have; (ii) the slightly *negative* row-effect, models moving the measured component *away* from $A^\dagger y$; (iii) the specific $528\times$ non-uniform CASSI spectrum, an operator fact rather than a consequence of sampling rate; and (iv) the cross-model, cross-modality *agreement* of the split. The ledger is accounting, not accusation: it locates where each pipeline's quality is manufactured, and the governed variants of §6.1 show the same quality can be retained at exact record consistency. Positioning against the range–null and null-space-injection prior art is §7 and Appendix D.
...
| Shang et al. (Bayesian SPI); conformal UQ | GT-free *statistical* uncertainty of DL reconstructions | Exact operator-level record certificate; measured-vs-prior split |
...
**Nearest prior art.** Three lines sit closest, and we separate ours from each by kind, not degree. *The null-space reconstruction cluster.* RND-SCI (Wang et al. 2023) already places the range–null decomposition *inside* CASSI, reconstructing the snapshot as $x = A^\dagger y + (I - A^\dagger A)q$ with a network that synthesizes only the null term; NPN/GSNR and the null-space networks of Schwab et al. (2019), together with DDNM, do the same on other operators. Every method in this cluster uses $A P_0 = 0$ to install the *intended* content in $\ker A$ and reads consistency-by-construction as a fidelity virtue. We invert that reading: the identical $A P_0 = 0$ is exactly why the record cannot certify what they write there, so we meter that content rather than trust it. *Hallucination limits.* Iagaru et al. (2026) bound hallucination through feasible-set diameters and Bhadra et al. (2021) map it through the same projectors, but both are worst-case or ground-truth-referenced statements about the operator; ours is a closed-form, per-mode, ground-truth-free contraction $c_i(\lambda)=\lambda/(\lambda+\sigma_i^2)$ evaluated at test time on the record actually in hand, plus a *constructed* cross-class witness that matches that record more tightly than the noisy truth — a per-instance object, not a bound. Gottschling et al. (2025) prove kernel-aware no-free-lunch theorems with destabilizing kernel constructions; ours are cross-class, record-tight, and displayed rather than existential. *Reliability and uncertainty.* Closest in intent is the trust line for learned single-pixel imaging (surveyed by Chen et al. 2026): Shang, O'Brien & Luke (2021) approximate the uncertainty of deep single-pixel reconstructions with a Bayesian CNN when the ground truth is unknown, localizing likely error to image edges via Monte-Carlo dropout. That estimate is statistical and model-dependent (finite training data and network); ours is complementary and different in kind — an exact, operator-level, ground-truth-free record-consistency certificate with a per-mode contraction, plus a measured-vs-prior attribution. Statistical uncertainty and geometric certificate answer the same deployment question — is this reconstruction trustworthy without a reference — from two sides. The dial's nearest interfaces, CodeFormer's $w$ and ESRGAN's $\alpha$, act on entangled features and move reconstructions off the measurement manifold; $B$ moves strictly in $P_0$ and holds $A\hat{x}_B = y$ at every setting. The extended review — hallucination benchmarks, instability, diffusion solvers, conformal UQ, identification theory across fields — is Appendix D.

**What survives prior-art subtraction.** After removing everything the range–null decomposition, null-space injection, and the Tikhonov filter complement already own, our contribution is exactly the pairing of a *ground-truth-free, closed-form per-mode audit* (not a ground-truth hallucination map, not a worst-case feasible-set bound, not a statistical uncertainty estimate), a *constructive cross-class witness* that matches the record tighter than the noisy truth on the estimator's own operator, and a *third-party quantitative attribution ledger* run over many published pipelines on their own released operators — whose contingent content (the $0.9$–$12\%$ measured record-inconsistency of those pipelines, the slightly negative row-effect, and the $528\times$ non-uniform CASSI spectrum that flat masked-Fourier MRI cannot exhibit) is not entailed by undersampling, and so is the finding rather than a tautology.

---

## 8. Discussion and Limitations

**The synthesis.** Report both axes and label provenance: the $m$ measured degrees of freedom are *measured* (certifiable by the audit); the injected null components are *prior-supplied* (exact against the record by construction, and openly attributed); the true null content is *unverifiable* from the record. These three labels are the whole discipline. $A\hat{x} = y$ certifies that a reconstruction reproduces the buckets; it never certifies that invented texture is the true scene.

**Hard limitations.** Every operator is simulation- or declared-operator-level. The GI study is simulated; the MRI leg is *emulated single-coil* (real multi-coil would need estimated ESPIRiT sensitivities and forfeit the exact $A = MF$ identity); the CASSI leg uses the *declared integer-shift* operator on the real released aperture, not a physical dispersive capture. Each leg buys exactness by stepping back from the physically-captured sensor: there is no photon-level noise model — bucket noise is Gaussian, a first-order proxy for the signal-dependent Poisson statistics of photon counting — and no hardware validation, so the certificates concern the *model* of the sensor. The clinical framing is honest about its own reach: at $4$–$8\times$ MRI the finding is coarse-certified, fine-detail-uncertified, with no reader study behind the pathology-box statistics. The true null content remains uncertifiable by any within-record method — that is the theorem, not an engineering gap.
...
- Cai, Y., Lin, J., Hu, X., Wang, H., Yuan, X., Zhang, Y., Timofte, R. & Van Gool, L. Mask-guided Spectral-wise Transformer for Efficient Hyperspectral Image Reconstruction. In Proc. IEEE/CVF Conference on Computer Vision and Pattern Recognition (CVPR), 17502–17511 (2022). arXiv:2111.07910. [MST model zoo audited in §6/Appendix E.3.]
...
- Chen, W. et al. Single-pixel imaging: principles, methods, algorithms, and applications. Advanced Imaging (2026). doi:10.3788/AI.2026.20002. [Review of the learned single-pixel-imaging line.]
...
- Meng, Z., Ma, J. & Yuan, X. End-to-End Low Cost Compressive Spectral Imaging with Spatial-Spectral Self-Attention. In Computer Vision – ECCV 2020, Lecture Notes in Computer Science 12368, 187–204 (Springer, 2020). [TSA-Net; source of the real released coded aperture and captures.]
...
- Shang, R., O'Brien, M. A. & Luke, G. P. Deep-learning-driven Reliable Single-pixel Imaging with Uncertainty Approximation. arXiv:2107.11678 (2021).
...
- Wang, J., Wang, S., Zhang, R., Zheng, Z., Liu, W. & Wang, X. A Range-Null Space Decomposition Approach for Fast and Flexible Spectral Compressive Imaging. arXiv:2305.09746 (2023). [RND-SCI.]
...
- Zbontar, J., Knoll, F., Sriram, A., Murrell, T., Huang, Z., Muckley, M. J., Defazio, A., Stern, R., Johnson, P., Bruno, M., et al. fastMRI: An Open Dataset and Benchmarks for Accelerated MRI. arXiv:1811.08839 (2018). [fastMRI single-coil knee $k$-space, §6/Appendix E.2.]
...
- Zhao, R. et al. fastMRI+, Clinical pathology annotations for knee and brain fully sampled magnetic resonance imaging data. Scientific Data 9, 152 (2022). [fastMRI+ pathology boxes, §6/Appendix E.2.]
...
### A.1 Forward model and compact SVD
...
### A.3 The Lemma: measurements constrain only the row space
...
### A.4 What can and cannot be certified
...
- **Not certifiable (the $n - m$ null degrees of freedom).** The component $P_0 x$ is unconstrained by $y$. The measurement supplies no evidence about what lives there; any content placed in the null space is a modeling choice, not a verified fact. This is the barrier of main §2 and, read constructively, the safe channel for the governed injection of main §4.
...
### A.5 The rows-orthonormal fusion case
...
### B.1 A per-instance feasible-but-wrong witness
...
### B.2 The record is matched more tightly than the truth
...
The box-constrained (POCS) witness experiment of main §2.1 uses this same operator; per-pair boxed residuals: 8 of 16 at machine precision, 9 of 16 below $10^{-11}$, worst pair $1.5\times10^{-3}$ (artifact: domain_diagnostics.json).


Crucially, the construction is not tied to that operator: on the *same* $64\times64$ STL10 operator at $5\%$ sampling ($m=205$) used for the governed dial of main §4, 40 feasible-wrong witnesses reproduce their target records at $\mathrm{RelMeasErr}(u_{ij}, y_i)$ between $1.7\times10^{-14}$ and $5.6\times10^{-13}$ (median $9.9\times10^{-14}$) while remaining semantically wrong (median $\mathrm{PSNR}(u_{ij}, x_i) = 19.5$ dB). The converse of this section and the constructive dial of main §4 are therefore exhibited on one and the same operator; the residual is a few orders of magnitude above the Rad-5 figure only because the mixed-basis operator is applied through a ridge-regularized ($\lambda_{\text{solver}}=10^{-6}$) audit rather than an exact orthonormal projector, and it remains ten orders below the $\sim\!10^{-3}$ noise floor.
...
### B.3 Position relative to the limits literature
...
### B.4 The unmeasured-content magnitude is not an error map
...
### B.7 Exact per-mode contraction
...
### B.8 Post-hoc audit across reconstructor families
...
### B.9 Position: exact modal spectrum, not a feasible-set bound
...
### B.10 The separation law in full
...
### B.11 The orthogonal error split
...
### B.13 The range-share law tracks the sampling rate
...
### B.14 What accountability catches that PSNR cannot
...
### B.15 Statement of the separation law
...
**Table C1. Locked-split absolute means (512 images, mean over 3 seeds).** Lower is better for LPIPS, RMSE, RAPSD; higher is better for PSNR, SSIM. Measurement fidelity is $\mathrm{RelMeasErr}\sim10^{-7}$ for every row.
...
### C.2 A pre-specified, one-shot locked confirmation
...
The one-shot property attaches to the pre-specified *gate evaluation*: the eight conditions were evaluated exactly once on the locked split. Later development-level analyses (the noise sweep of main §4.4 and the breadth analysis of main §4.4) re-read the same locked images with the dial frozen and *no further selection of any kind*; they postdate the gate, change nothing upstream of it, and are labeled development-level precisely because the split's one-shot statistical guarantee does not extend to them.
...
### C.3 The dial is a fixed scalar, not a per-image oracle
...
### C.4 The no-adaptation lemma and the audit tax
...
**Table C2. Cross-rate generalization (development-level, 3 seeds per rate).**
...
### C.7 Noise robustness (development-level)
...
**Table C3. Noise robustness, LPIPS by bucket-noise level (development-level, 3 seeds, locked split, $B$ frozen).**
...
### C.8 Breadth of improvement and its failure mode
...
### D.1 Ghost imaging and single-pixel imaging
...
### D.2 Range–null and data-consistency reconstruction
...
### D.3 Generative and VQ priors for inverse problems
...
### D.4 Perception–distortion tradeoffs and perceptual metrics
...
### D.5 Diffusion inverse-problem solvers
...
### D.6 Hallucination, instability, and fundamental limits
...
### D.8 Cross-field framings: identifiability, information theory, and Bayesian inversion
...
**Fourth**, we convert null-space injection from an all-or-nothing fill into a single-scalar, pre-specified **governed dial** that fuses VQAE structure and VQGAN detail and keeps $A\hat{x}_B = y$ *exact* for every $B$ behind a locked gate — moving the field from *assessment* (does a reconstruction hallucinate?) to *governance* (how much invented detail do we add, where the bucket is provably blind, without ever breaking the measurement or reporting the addition as measured evidence?).
...
**Table E1. Forensics summary (own operator, own code).**
...
We stress the reading: this is not an accusation of error — the null content these methods supply is often substantially truth-aligned — it is the *location* of their gains in the two-ledger account, measured on their own instruments. The separation law of main §3.3 describes the field, not just our operator.

### E.2 Accelerated MRI: masked-Fourier forensics on real fastMRI

The three pipelines above are ghost-imaging-native. To test the two-ledger reading on a real, second-modality operator — not our speckle construction, and not a simulation — we ran the same decomposition on real NYU fastMRI knee $k$-space with the exact masked-Fourier operator $A = MF$ (single-coil emulated, $M$ the released undersampling mask), and on the **official** fastMRI single-coil U-Net (chans $= 256$, wired from the released state dict). The operator is exact, so no calibration error is inherited: the projector identities hold in float64 on the real $k$-space to $\sim\!10^{-17}$, and the ESC ground truth $\lvert\operatorname{crop}(\operatorname{ifft2c}(k))\rvert$ agrees to correlation $1.00000$. Because $A = MF$ is a masked Fourier operator, the zero-filled reconstruction *equals* $P_R x_{\mathrm{gt}}$ exactly; its magnitude PSNR therefore **is** the range ceiling, with no SVD reference needed.
...
[FIG: FORENSICS_MRI_8x.pdf | 1.0 | **The two ledgers on real fastMRI at $R{=}8$.** The official single-coil U-Net's $+1.09$ dB over the range ceiling is $92\%$ null-supplied; the zero-filled image *is* $P_R x_{\mathrm{gt}}$, so the ceiling is exact, not a min-norm proxy. The governed data-consistent variant keeps the gain at exact record consistency.]

**Table E3. fastMRI single-coil U-Net decomposition (real $k$-space, official state dict).** *Ceiling* is the exact magnitude PSNR of $P_R x_{\mathrm{gt}}$ (zero-filled). *U-Net drift* is the measured record inconsistency $\lVert A\hat{x}-y\rVert/\lVert y\rVert$. *Governed DC* is $A^\dagger y + P_0 x_{\mathrm{UNet}}$; its own record drift is $\le 3.5\times10^{-16}$ at both rates (exact by construction).
...
The pattern of Appendix E.1 recurs on a modality that shares nothing with ghost imaging but the geometry: the U-Net's improvement over the ceiling is $91$–$92\%$ null-supplied, and the network is $3$–$6\%$ record-*inconsistent* — it moves the measured component off $A^\dagger y$. That inconsistency is the contingent content (a data-consistent estimator would have none): the governed variant $A^\dagger y + P_0 x_{\mathrm{UNet}}$ keeps nearly all the gain ($29.42$ vs $29.68$ dB at $R{=}4$) at exact record consistency (drift $3.5\times10^{-16}$), buying certifiability for $0.26$ dB.
...
The reading is near-uniform: twelve of the thirteen models supply $100$–$101\%$ of their PSNR gain over the $19.04$ dB ceiling from the null space, with the row-effect slightly **negative** — the models mildly corrupt the measured component rather than repairing it. GAP-Net is the sole exception ($93\%$ null, a positive row-effect, and the largest drift, $11.9\%$). The entire decade of CASSI progress (TSA-Net $31.67\to$ DAUHST-9stg $38.18$, $+6.5$ dB of SOTA) happens *entirely* in the null ledger, and the models are $0.9$–$12\%$ record-inconsistent throughout.

**The tautology defense, stated where it bites hardest.** At $m\ll n$ the null-dominance of a record-consistent estimator's error is definitional ($AP_0=0$ plus the orthogonal split), not a finding, and we do not present it as one (main §5, §6.3). What is *contingent* — not entailed by undersampling, and therefore the content — is fourfold: (i) the measured record-inconsistency of the published models ($0.9$–$12\%$), which a data-consistent estimator would not have; (ii) the slightly *negative* row-effect, models moving the measured component away from $A^\dagger y$; (iii) the specific $528\times$ non-uniform CASSI spectrum; and (iv) the cross-model agreement of the split over a whole zoo. The ledger is accounting, not accusation: the null content these models supply is largely truth-aligned. Its *location* — entirely in the uncertifiable ledger, on the field's own operator — is the finding.

---

## Appendix F. Negative results

### F.1 Abandoned negative result: posterior $z$-sampling diversity
