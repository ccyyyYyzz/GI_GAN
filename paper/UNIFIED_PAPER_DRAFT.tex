...
\textit{On the map (Fig. 1): the} cannot \textit{branch.}
...
\textit{On the map: the} can \textit{branch --- what the same geometry still permits.}
...
\caption{\textbf{The certificate and the separation.} \textit{(a)} Each measured mode's residual contracts by the closed-form factor $\lambda/(\lambda+\sigma_i^2)$ --- smaller $\lambda$, harder projection. \textit{(b)} Applied post-hoc to four unrelated reconstructors, the audit moves measurement accountability by orders of magnitude while PSNR barely moves (green): the two axes are decoupled exactly because $AP_0 = 0$.}
\end{figure}
...
\textit{On the map: still the} can \textit{branch --- what the certificate does and does not buy.}
...
\textit{On the map: the} therefore \textit{--- the converse licenses injection exactly where the certificate cannot reach.}
...
\textit{On the map: still the} therefore \textit{--- choosing, locking, and stress-testing one operating point.}
...
\textit{On the map: the outer branches --- how far the locked point travels, and what the frame says about the published field.}
...
We stress the reading: this is not an accusation of error --- the null content these methods supply is often substantially truth-aligned --- it is the \textit{location} of their gains in the two-ledger account, measured on their own instruments. The separation law of §5 describes the field, not just our operator.
...
\textbf{Fourth}, we convert null-space injection from an all-or-nothing fill into a single-scalar, pre-registered \textbf{honesty dial} that fuses VQAE structure and VQGAN detail and keeps $A\hat{x}_B = y$ \textit{exact} for every $B$ behind a locked gate --- moving the field from \textit{assessment} (does a reconstruction hallucinate?) to \textit{governance} (how much invented detail do we add, where the bucket is provably blind, without ever breaking or laundering the measurement?).
