...
\begin{enumerate}
\item \textbf{Constructive impossibility.} For any target record and donor image, we construct a feasible-but-wrong witness that preserves the target bucket record while importing the donor's null-space content (§2).
\item \textbf{Exact record-consistency audit.} A post-hoc audit whose residual in each measured singular mode contracts by exactly $\lambda/(\lambda+\sigma_i^2)$, certifying agreement with the recorded bucket independently of the reconstructor (§3).
\item \textbf{Governed null-space supply.} A scalar dial that fuses prior detail strictly inside $\ker A$, preserving $A\hat{x}_B = y$ for every setting and improving locked LPIPS by 32.6\% at a bounded distortion cost (§4).
\item \textbf{Limits and external validity.} A no-adaptation lemma for ground-truth-free dial selection, and a projector audit of three published DL-GI pipelines on their own released or measured operators (§4.5, §5).
\end{enumerate}
...
The witness is also a \textit{legal image}, not only a linear-space vector. The raw splice leaves the pixel box mildly (range $[-0.34, 1.42]$, median $3\%$ of pixels outside $[0,1]$ on the dial's operator), and naive clipping does break its residual (to ${\sim}10^{-2}$). Alternating the exact fiber projection with box clipping, however, converges to a \textbf{box-constrained witness}: a $[0,1]$-valued image matching the record at a median $3.4\times10^{-12}$ (max $1.5\times10^{-3}$ over 16 pairs) while carrying the donor's null content (median PSNR to donor $9.3$ dB). The impossibility is not an artifact of unconstrained linear algebra.
...
\textbf{Theorem 3 (exact consistency for every $B$).} $A\hat{x}_B = A x_0 + A P_0(\cdot) = y$ \textit{for all} $B \in \mathbb{R}$. The identity that makes null content unverifiable (§2) is the identity that makes injecting it measurement-safe. Numerically the locked splits hold $\mathrm{RelMeasErr}$ at $3.6\times10^{-7}$ (float32 pipeline scale) for every operating point.
...
\textbf{Lemma 2 (no-adaptation).} \textit{Any selection rule $g(A, y)$ computable from the record alone is constant on each fiber $\{x: Ax = y\}$. Consequently, under squared loss the best ground-truth-free selector is the conditional mean of the oracle over the record,}
...
\textit{so the irreducible adaptation error is the oracle's conditional variance over the fiber.} (Proof: fiber-mates share $y$, hence share $g$; the identity is the conditional-expectation projection theorem. $\blacksquare$)
...
\textbf{Hard limitations.} The study is simulation-only, at one resolution, with locked claims on one 5\% operator; there is no photon-level noise model and no hardware validation. The true null content remains uncertifiable by any within-record method --- that is the theorem, not an engineering gap.
...
\begin{itemize}
\item \textbf{Certifiable (the $m$ row coordinates).} The component $P_R x = A^\dagger y$ is fixed by the record: any measurement-consistent reconstruction must have $P_R \hat{x} = A^\dagger y$, and re-measuring it reproduces $y$. These $m$ coordinates are pinned down and can be audited without ground truth (main §2).
...
\subsection*{The separation law in full}
...
\subsection*{7.2 A pre-specified, one-shot locked confirmation}
...
The one-shot property attaches to the pre-specified \textit{gate evaluation}: the eight conditions were evaluated exactly once on the locked split. Later development-level analyses (the noise sweep of main Appendix A and the breadth analysis of main Appendix A.3) re-read the same locked images with the dial frozen and \textit{no further selection of any kind}; they postdate the gate, change nothing upstream of it, and are labeled development-level precisely because the split's one-shot statistical guarantee does not extend to them.
...
\subsection*{Robustness studies in full}
