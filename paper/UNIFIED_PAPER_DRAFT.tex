...
> \textbf{Scope of all certificates.} In this paper, "certified" always means certified agreement with the recorded bucket $y$. It never means certified agreement with the unknown scene. All content in $P_0$ is prior-supplied unless additional measurements are acquired. Claims are tiered: \textit{locked} results follow a pre-registered one-shot protocol; \textit{development-level} results reuse the frozen system without a pre-registered gate. This box governs every section below.
...
\subsection*{2.2 A wrong image passes the measurement test more tightly than the truth}
...
\subsection*{2.3 The observable null magnitude is a content map, not an error map}
...
\textbf{Theorem 2 (PSNR ceiling for row-space corrections).} \textit{Any correction $T$ with $T(v) - v \in \mathcal{R}(A^\top)$ raises PSNR by at most $-10\log_{10}(1-s)$; the bound is attained by the hard projection under noiseless $y$ and not attained under noise. Corrections that edit the null space are outside the hypothesis --- which is the content: only null-space injection can exceed the row budget, and the measurement certifies none of it.}
...
The dial is not a fusion architecture. It is a provenance constraint: whatever prior is used, its contribution is projected into $\ker A$, so perceptual detail is supplied without borrowing measurement authority. We call the resulting control \textit{governed} --- and, once, \textit{honest} --- because it changes only what the measurement cannot certify and reports that change as prior-supplied detail.
...
\textbf{Table 1. Locked-split means (512 images, 3 seeds).}
...
\caption{\textbf{Three paradigms, three operators, one structure.} \textit{(a)} The fine-tuning trajectory decomposed per step: PSNR rises past the operator's range ceiling --- everything above it is paid from the null ledger. \textit{(b)} Attribution of each method's improvement: row repair vs null injection. \textit{(c)} Every headline stands $+3.1$ to $+8.1$ dB above its own range ceiling with terminal error $93$--$97\%$ null.}
\end{figure}
...
\textit{(The appendices preserve the long-form material of the working draft; internal section numbers refer to its original structure where they appear.)}
