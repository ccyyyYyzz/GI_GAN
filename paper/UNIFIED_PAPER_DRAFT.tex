...
\usepackage{parskip}
\title{Certify What You Measure, Govern What You Cannot: A Range--Null Account of Accountability and Governed Detail in Undersampled Ghost Imaging}
\author{}\date{}
\begin{document}
\maketitle
...
\subsection*{1.1 The governing fact}
...
\textbf{Converse (the barrier, §3).} Because $y$ fixes exactly $P_R x$ and nothing else, null-space content is \textit{provably} unverifiable from the measurement. We make this constructive rather than abstract: for any two images $x_i, x_j$ the spliced image $u_{ij} = x_j - A^\dagger(A x_j - y_i)$ carries the measured component of $x_i$ but the null-space content of $x_j$, and satisfies the record of $x_i$ to floating-point precision. Empirically these witnesses match the record at floating-point round-off --- tighter than the noisy truth matches its own record --- while remaining semantically wrong (all constructed pairs; numbers in §3.2). Consistency is not correctness.

\textbf{Achievability (the certificate, §4).} The same geometry that forbids certifying $P_0 x$ \textit{does} permit a ground-truth-free, test-time audit of the measured component. Attaching $\Pi_y^\lambda(v) = v - A^\top(A A^\top + \lambda I)^{-1}(A v - y)$ to any reconstructor contracts each measured singular mode by exactly $\lambda/(\lambda + \sigma_i^2)$ and leaves $P_0 v$ untouched. The audit reduces measurement residuals by orders of magnitude across analytic, variational, and learned reconstructors at negligible PSNR cost, cleanly separating image quality from measurement accountability, and matches the closed-form contraction at the float64 floor (§4).

\textbf{Governed use (the dial, §6--§8).} Since no measurement can certify the null space, the honest response is not to hide prior-supplied content but to inject it precisely where it is provably invisible to the bucket, and to meter it. Anchoring at a measurement-audited LMMSE estimate $x_0$ (so $A x_0 = y$) and fusing the null-space differences of a reconstruction prior (VQAE, contribution $d_A$) and an adversarial prior (VQGAN, contribution $d_G$) through a single scalar $B$,

$$\hat{x}_B = x_0 + P_0\big(d_A + B(d_G - d_A)\big),$$

gives $A \hat{x}_B = y$ \textit{exactly} for every $B$. At the balanced operating point this improves locked LPIPS by 32.6\% at a bounded PSNR cost, passing an 8/8 pre-registered acceptance gate on a raw-hash-disjoint locked split (§7).

\subsection*{1.3 Overview figure}

Figure 1 renders this chain as three panels --- \textit{cannot / can / therefore}. The left panel (\textbf{cannot}) shows a feasible-but-wrong pair: a true target and its spliced counterpart, sharing the measured row space and differing only in the null space, whose bucket residual ($2.93\times10^{-15}$) is smaller than the truth's own noisy residual ($5.36\times10^{-3}$) --- a measurement-consistent, semantically wrong, non-natural image. The middle panel (\textbf{can}) shows the exact per-mode certificate: the audit contracting each measured singular mode by $\lambda/(\lambda+\sigma_i^2)$, driving residuals to the noise floor while PSNR barely moves. The right panel (\textbf{therefore}) shows metered injection at $B = 0.55$, with the fused reconstruction stamped $A\hat{x} = y$ --- invented detail placed only where the bucket is blind, without ever breaking the measurement.

\begin{figure}[t]\centering
\includegraphics[width=1.0\linewidth]{METHOD_FIG1.pdf}
\caption{\textbf{the deductive chain as three panels.} \textit{Cannot:} a feasible-but-wrong witness reproduces the target's bucket record to $2.93\times10^{-15}$ relative error --- tighter than the noisy truth's own $5.36\times10^{-3}$ --- while depicting a different scene; null-space content is unverifiable from the record. \textit{Can:} the ground-truth-free audit $\Pi_y^\lambda$ contracts each measured singular mode by exactly $\lambda/(\lambda+\sigma_i^2)$, driving the measurement residual by orders of magnitude at negligible PSNR cost. \textit{Therefore:} prior detail is injected only inside the null space through the dial $B$, and $A\hat{x}_B = y$ holds exactly at every setting.}
\end{figure}

\subsection*{1.4 Contributions}

\begin{enumerate}
\item \textbf{A constructive converse.} We turn "consistency is not correctness" from an abstract statement into a per-instance, feasible-but-wrong witness that matches the recorded bucket to $\sim 2\times10^{-15}$ --- tighter than the noisy truth --- establishing empirically that null-space content is unverifiable.
\item \textbf{A ground-truth-free measurement certificate.} A plug-in test-time audit $\Pi_y^\lambda$ that contracts each measured mode by exactly $\lambda/(\lambda + \sigma_i^2)$, applies uniformly across BP, Tikhonov, CS-TV, and learned reconstructors, and separates image quality from measurement accountability (residuals drop orders of magnitude at $|\Delta\mathrm{PSNR}| \le 0.039$ dB).
\item \textbf{A governed, measurement-safe injection dial.} A single scalar $B$ fusing VQAE structure and VQGAN detail strictly inside $P_0$, keeping $A\hat{x}_B = y$ exact for every $B$, delivering a locked balanced LPIPS gain of $-0.0977$ (32.6\%; KID $0.119 \to 0.043$) behind a pre-registered gate, and tracing a monotone perception--distortion ladder (LMMSE $0.404 \to$ VQAE $0.300 \to$ balanced $0.202 \to$ quality-lite $0.182 \to$ VQGAN $0.172$).
\item \textbf{A no-adaptation lemma, with its empirical sharpness.} Any selection rule computable from one record is constant on each measurement fiber (an elementary corollary of the converse we state formally in §7.4), so ground-truth-free per-image adaptation reduces to the $y$-predictable component of the oracle --- which, on this benchmark [development-level], is worth $\le 0.002$ LPIPS beyond a matched-PSNR constant (feature--oracle correlations $|\rho|\le0.24$). The honest dial is a scalar: exactly so across each fiber, empirically so on this dataset.
\item \textbf{Projector forensics of published pipelines.} We decompose the reported gains of three published DL-GI methods --- pretrained+fine-tuned (Wang et al. 2022a), untrained DIP on real measured speckle (GIDC), and self-supervised on a noisy record (Noise2Ghost) --- on \textbf{their own operators with their own code}, and find one law: the certifiable row ledger saturates (terminal error $93$--$97\%$ null) and every headline PSNR stands $+3.1$ to $+8.1$ dB above its own operator's range ceiling, paid from the null space (§8.4). The separation law describes the field, not just our construction.
\end{enumerate}

\subsection*{1.5 Non-claims}

We state the boundaries of the work up front and honor them throughout. This is \textbf{not} a state-of-the-art result; we do \textbf{not} claim to beat diffusion-based solvers, and we report \textbf{no} hardware experiments. The fusion weight $B$ is a fixed, validation-selected operating point, \textbf{not} a per-image oracle. Posterior null-space $z$-diversity was pursued and is reported only as an abandoned negative result, never as a positive claim. The cross-rate (2\%, 10\%) and measurement-noise studies are \textbf{development-level} evidence, not locked claims. The observable magnitude $|P_0 \hat{x}|$ is \textbf{not} a pixelwise error locator. And most importantly: measurement consistency means $A\hat{x} = y$ and nothing more --- the bucket \textbf{never} certifies that the injected texture is the true scene.

\medskip\hrule\medskip

\section*{2. The Range--Null Geometry}
...
\subsection*{2.1 Forward model and compact SVD}

We consider the standard linear ghost-imaging model
$$
y = A x + \textbackslash\{\}varepsilon,
$$
...
$$
A = U\_r \textbackslash\{\}Sigma\_r V\_r\textasciicircum{}\textbackslash\{\}top, \textbackslash\{\}qquad r = \textbackslash\{\}operatorname\{rank\}(A),
$$
...
\subsection*{2.2 The two projectors}
...
$$
P\_R = A\textasciicircum{}\textbackslash\{\}dagger A = V\_r \textbackslash\{\}Sigma\_r\textasciicircum{}\{-1\} U\_r\textasciicircum{}\textbackslash\{\}top U\_r \textbackslash\{\}Sigma\_r V\_r\textasciicircum{}\textbackslash\{\}top = V\_r V\_r\textasciicircum{}\textbackslash\{\}top,
\textbackslash\{\}qquad
P\_0 = I - V\_r V\_r\textasciicircum{}\textbackslash\{\}top.
$$
Because $V_r^\top V_r = I_r$, each is symmetric and idempotent, and the two are complementary and mutually annihilating:
$$
P\_R\textasciicircum{}\textbackslash\{\}top = P\_R, \textbackslash\{\}quad P\_R\textasciicircum{}2 = P\_R, \textbackslash\{\}qquad
P\_0\textasciicircum{}\textbackslash\{\}top = P\_0, \textbackslash\{\}quad P\_0\textasciicircum{}2 = P\_0, \textbackslash\{\}qquad
P\_R + P\_0 = I, \textbackslash\{\}quad P\_R P\_0 = 0 .
$$
They are therefore the pair of orthogonal projectors onto the two orthogonal subspaces
$$
\textbackslash\{\}mathbb\{R\}\textasciicircum{}n = \textbackslash\{\}mathcal\{R\}(A\textasciicircum{}\textbackslash\{\}top) \textbackslash\{\}oplus \textbackslash\{\}mathcal\{N\}(A), \textbackslash\{\}qquad \textbackslash\{\}mathcal\{R\}(A\textasciicircum{}\textbackslash\{\}top) \textbackslash\{\}perp \textbackslash\{\}mathcal\{N\}(A),
$$
the row space (range of $A^\top$) and the null space (kernel of $A$). Every image splits uniquely and orthogonally,
$$
x = P\_R x + P\_0 x, \textbackslash\{\}qquad \textbackslash\{\}langle P\_R x, \textbackslash\{\}, P\_0 x \textbackslash\{\}rangle = x\textasciicircum{}\textbackslash\{\}top P\_R P\_0\textbackslash\{\}, x = 0,
$$
so the measured and unmeasured parts share no energy.

\subsection*{2.3 The Lemma: measurements constrain only the row space}
...
$$
A P\_0 = 0 \textbackslash\{\}qquad\textbackslash\{\}text\{and\}\textbackslash\{\}qquad A P\_R = A .
$$

\textit{Proof.} Using the Moore--Penrose identity $A A^\dagger A = A$,
$$
A P\_0 = A\textbackslash\{\},(I - A\textasciicircum{}\textbackslash\{\}dagger A) = A - A A\textasciicircum{}\textbackslash\{\}dagger A = A - A = 0,
$$
...
$$
A x = A P\_R x + A P\_0 x = A P\_R x ,
$$
...
$$
\textbackslash\{\}\{\textbackslash\{\}, x : A x = y \textbackslash\{\},\textbackslash\{\}\} = A\textasciicircum{}\textbackslash\{\}dagger y + \textbackslash\{\}mathcal\{N\}(A) .
$$
...
\subsection*{2.4 What can and cannot be certified}
...
\begin{itemize}
\item \textbf{Certifiable (the $m$ row coordinates).} The component $P_R x = A^\dagger y$ is fixed by the record: any measurement-consistent reconstruction must have $P_R \hat{x} = A^\dagger y$, and re-measuring it reproduces $y$. These $m$ coordinates are pinned down and can be audited without ground truth (§4).
\item \textbf{Not certifiable (the $n - m$ null coordinates).} The component $P_0 x$ is unconstrained by $y$. The measurement supplies no evidence about what lives there; any content placed in the null space is a modeling choice, not a verified fact. This is the barrier of §3 and, read constructively, the safe channel for the governed injection of §6.
\end{itemize}
...
\subsection*{2.5 The rows-orthonormal fusion case}
...
$$
A\textasciicircum{}\textbackslash\{\}dagger = A\textasciicircum{}\textbackslash\{\}top, \textbackslash\{\}qquad P\_R = A\textasciicircum{}\textbackslash\{\}top A, \textbackslash\{\}qquad P\_0 = I - A\textasciicircum{}\textbackslash\{\}top A,
$$
...
\section*{3. The Verifiability Barrier (Converse)}
...
\subsection*{3.1 A per-instance feasible-but-wrong witness}
...
\subsection*{3.3 Position relative to the limits literature}
...
\subsection*{3.4 The unmeasured-content magnitude is not an error map}
...
\section*{4. The Accountability Certificate (Achievability)}
...
\subsection*{4.1 A plug-in test-time audit}
...
\subsection*{4.2 Exact per-mode contraction}
...
\subsection*{4.3 Post-hoc audit across reconstructor families}
...
\subsection*{4.4 Position: exact modal spectrum, not a feasible-set bound}
...
\section*{5. Quality Is Not Accountability: The Separation Law}
...
\subsection*{5.1 The orthogonal error split}
...
The measurement sees only the first term: $A e = A P_R e = A(\hat{x} - x) = A\hat{x} - y$, so the audit residual $\|A\hat{x}-y\|$ is a function of $P_R e$ alone and is entirely insensitive to $P_0 e$. Pixel error, in contrast, integrates \textit{both} terms. Define the pre-audit row-error share
...
\subsection*{5.2 The PSNR ceiling}
...
\subsection*{5.3 The range-share law tracks the sampling rate}
...
\subsection*{5.4 What accountability catches that PSNR cannot}
...
\subsection*{5.5 Statement of the separation law}
...
$$\hat{x}_B = x_0 + P_0\big(d_A + B\,(d_G - d_A)\big).$$
The scalar has a clean reading: $B = 0$ returns the VQAE structure reconstruction $x_0 + P_0 d_A$, $B = 1$ returns the VQGAN detail reconstruction $x_0 + P_0 d_G$, and intermediate $B$ trades structure fidelity for adversarial detail. Consistent with the program's non-claims, $B$ is a \textit{fixed, validation-selected} operating point --- pre-registered and frozen per seed, not adapted per image and not an oracle that knows the best weight for each scene.

\subsection*{6.3 The consistency theorem: exact, for every $B$}

\textbf{Theorem.} For every $B \in \mathbb{R}$, the fused reconstruction satisfies $A\hat{x}_B = y$ exactly.

\textit{Proof.} Apply $A$ and use linearity:
...
We restate the scope deliberately, because the firewall of this program depends on it. $A\hat{x}_B = y$ certifies that the reconstruction \textit{reproduces the buckets} --- nothing more. The injected texture lives entirely in the null space, where the measurement furnishes no evidence; it is a prior-supplied hypothesis about plausible content, never a measurement-certified fact. The dial governs \textit{how much} invented detail to supply; it never converts that detail into truth. Section 7 characterizes where along this dial the useful operating points lie, and §9 returns to why the accountable row space and the responsibly supplied null space must never be read in the same currency.

\medskip\hrule\medskip

\section*{7. The Honesty Dial}

The consistency theorem of §6 licenses a construction but does not, by itself, decide how much invented detail to admit. The fusion weight $B$ is a free scalar: every value produces a reconstruction that reproduces the record exactly, so the measurement offers no guidance on where along the family $\hat{x}_B = x_0 + P_0\big(d_A + B(d_G - d_A)\big)$ to stand. This is the point of the construction, not a defect of it --- the geometry $A P_0 = 0$ that makes null-space content unverifiable (the converse of §3) is the same geometry that makes injecting it measurement-safe (the consistency theorem of §6). What remains is to choose $B$ honestly, report the trade it buys, and pre-commit to the choice before touching the confirmatory data. We call the resulting object an \textit{honesty dial}: a single scalar that meters null-space detail, moves along a perception--distortion frontier, and never converts the perceptual gain it produces into a measurement claim.

\subsection*{7.1 A metered perception--distortion ladder}
...
\textbf{Table 1. Locked-split absolute means (512 images, mean over 3 seeds).} Lower is better for LPIPS, RMSE, RAPSD; higher is better for PSNR, SSIM. Measurement fidelity is $\mathrm{RelMeasErr}\sim10^{-7}$ for every row.
...
\caption{\textbf{the metered perception--distortion ladder.} Named operating points of Table 1 on the perception--distortion plane (LPIPS versus PSNR/RMSE): the LMMSE anchor, the VQAE structure branch ($B{=}0$), the pre-registered balanced point ($B{\approx}0.55$), quality-lite ($B{\approx}0.72$), and full VQGAN ($B{=}1$). The ladder is monotone --- perceptual quality is bought at a metered pixel-fidelity cost --- and every point satisfies $A\hat{x}=y$ to $\mathrm{RelMeasErr}\sim10^{-7}$; the trade moves entirely within the null space.}
\end{figure}

\begin{figure}[t]\centering
\includegraphics[width=0.85\linewidth]{B_CURVE.pdf}
\caption{\textbf{the frontier is smooth, monotone, and interior-optimum-free.} A dense 21-point development-split sweep of the fusion weight $B$ (step $0.05$): LPIPS falls strictly monotonically from $0.293$ to $0.167$ while PSNR declines from $22.87$ to $21.17$ dB (one shallow interior maximum of $+0.03$ dB at $B=0.10$). No discontinuity and no interior perceptual optimum --- the dial is a stable interpolation, not a fragile hyperparameter.}
\end{figure}

\subsection*{7.2 A pre-registered, one-shot locked confirmation}
...
\subsection*{7.4 The no-adaptation lemma and the audit tax}
...
\section*{8. Robustness and Reach}

The locked claim of §7 fixes one operating point: the balanced dial $B$, at a single $5.0\%$ operator, on one $64\times64$ resolution, under noiseless acquisition. This section asks how far that operating point reaches --- across sampling rate, across measurement noise, and across the image population. All results here are \textbf{development-level}: they reuse the frozen system, re-select nothing on the locked split, and constitute supplementary evidence rather than a second pre-registered claim. We label them as such throughout and treat none of them as certified. The measurement-consistency guarantee, however, is not development-level: because $A P_0 = 0$ holds for every operator by construction, $A\hat{x}_B = y$ remains exact at every rate and every noise level below.

\subsection*{8.1 Cross-rate generalization (development-level)}
...
\textbf{Table 2. Cross-rate generalization (development-level, 3 seeds per rate).}
...
\caption{\textbf{the perceptual gain generalizes across sampling rate (development-level).} Balanced fusion's relative LPIPS gain over the VQAE branch at $2\%$, $5\%$ (locked), and $10\%$ sampling, with the PSNR cost within the pre-registered $-0.5$ dB validation tolerance at every rate (3 seeds per rate, all same-direction).}
\end{figure}

\subsection*{8.2 Noise robustness (development-level)}
...
\textbf{Table 3. Noise robustness, LPIPS by bucket-noise level (development-level, 3 seeds, locked split, $B$ frozen).}
...
\subsection*{8.3 Breadth of improvement and its failure mode}
...
\textbf{Table 4. Forensics summary --- each pipeline on its own operator, with its own code.}
...
\subsection*{9.1 A prior can raise naturalness without reducing distance to the true null content}
...
$$
\textbackslash\{\}hat\{x\}\_\{B\_1\} - \textbackslash\{\}hat\{x\}\_\{B\_2\} = P\_0\textbackslash\{\}big[(B\_1 - B\_2)(d\_G - d\_A)\textbackslash\{\}big] \textbackslash\{\}in \textbackslash\{\}mathcal\{N\}(A).
$$
Whatever a stronger prior buys --- the locked perceptual and distributional gains of §7 --- it buys entirely in $P_0$. The null-space error, however, is $P_0(\hat{x} - x) = P_0\hat{x} - P_0 x$, and its second term is the true scene's unmeasured content, the one quantity the record cannot reveal. A prior that increases naturalness changes $P_0\hat{x}$; it carries \textbf{no guarantee} of reducing the distance to $P_0 x$ --- a more realistic reconstruction may be nearer to, or farther from, the true null content than a flatter one, and the measurement provides no way to tell which (the prior-content map $|P_0\hat{x}|$ of §3.4 exhibits the same decoupling directly). Perceptual gain is real and is measured on the quality axis; it must never be read as evidence on the null axis, because $A P_0 = 0$ severs the two.

\subsection*{9.2 Prescription: report both axes, and label provenance}

The discipline that follows is procedural, and we adopt it throughout. First, \textbf{report measurement accountability and image quality as separate axes}, never one as a proxy for the other: RelMeasErr certifies agreement with the record; LPIPS, KID, and RAPSD describe perceptual quality; PSNR and RMSE describe pixel distortion. A high value on either axis licenses no inference about the other (§5). Second, \textbf{label every reconstructed coordinate by provenance}: the $m$ row-space coordinates are \textit{measured} --- pinned by $y$ and certifiable by the audit; the null-space coordinates carrying injected detail are \textit{prior-supplied} --- a modeling choice, exact against the record by $A P_0 = 0$, and openly attributed to the prior; and the true null content $P_0 x$ is \textit{unverifiable} --- no test on $y$ can certify it. These three labels --- \textbf{measured / prior-supplied / unverifiable} --- are the whole synthesis, and the boundary is stated without exception: $A\hat{x} = y$ certifies that a reconstruction reproduces the buckets; it never certifies that the injected texture is the true scene.
...
\subsection*{10.1 Ghost imaging and single-pixel imaging}
...
\subsection*{10.2 Range--null and data-consistency reconstruction}
...
\subsection*{10.3 Generative and VQ priors for inverse problems}
...
\subsection*{10.4 Perception--distortion tradeoffs and perceptual metrics}
...
\subsection*{10.5 Diffusion inverse-problem solvers}
...
\subsection*{10.6 Hallucination, instability, and fundamental limits}
...
\subsection*{10.8 Cross-field framings: identifiability, information theory, and Bayesian inversion}
...
\textbf{Our delta.} These fields supply the rigorous vocabulary we import and instantiate, and we own the lineage rather than reinvent it. Our feasible-but-wrong images are the imaging analogue of Koopmans \& Reiersøl's observational equivalence and Rothenberg's singular information direction; our null space $P_0 = I - A^\dagger A$ is Manski's identification region, Landau's forced kernel, and Barrett \& Myers's null-function subspace; our certifiable-functional split is Backus--Gilbert resolvability; and, most sharply, our governed dial is Lewbel's \textit{normalization} --- injecting null content is "without loss of generality" for the measurement, since $A\hat{x}_B = y$ exactly for every $B$, leaving the identified quantity untouched. Candès--Romberg--Tao's converse is the CS-side twin of the econometric one, and Stuart's "prior stays decisive" is the Bayesian mirror of our per-mode contraction, whose factor $\lambda/(\lambda+\sigma_i^2)$ is exactly a Gaussian-posterior variance reduction along each singular direction --- the same term MacKay aggregates into $\gamma$, which we instead read per record as a test-time audit. Our contribution is not a new scalar on $(A,\sigma)$: on the linear-Gaussian layer these objects are settled, and any quantity defined there is a renaming of one of them. It is the \textit{constructive converse}, the repurposing of the modal contraction as a ground-truth-free per-record certificate that separates quality from accountability, and the governed injection dial --- an operational stack, unified for undersampled ghost imaging, that these prior framings describe in pieces but never assemble as a governance instrument. The \textbf{no-adaptation lemma} (§7.4) is, as mathematics, an elementary measurability fact; we state it formally because the reconstruction literature keeps paying for its absence --- per-image adaptation schemes that cannot, even in principle, out-adapt their own global prior on a fixed record --- and because its value lies in the pairing with the empirical finding that the $y$-predictable remainder is worth essentially nothing here.

\subsection*{10.9 Novelty statement}

Every line above uses the range--null geometry, or a prior over the unmeasured content, to \textbf{reconstruct}: install the "correct" prior in the null space for fidelity (Wang et al., 2023 AAAI; RND-SCI; DDNM; MCG), learn to predict null content from $y$ (NPN; GSNR; Chen \& Davies, 2020), fabricate null-like error to benchmark detectors (HalluGen), quantify output uncertainty against calibration data (RCPS; conformal risk control; Wen et al., 2024), or prove abstractly that the kernel forces hallucination (Iagaru et al., 2026; Bhadra et al., 2021; Antun et al., 2020). We use the identical geometry to \textbf{certify, bound, and meter}, and our contribution rests on four airtight, uncontested hooks. \textbf{First}, we make the barrier \textit{constructive}: an explicit feasible-but-wrong witness that matches the same bucket record to machine precision --- tighter than the noisy truth --- turning "consistency is not correctness" from a theorem about maps (Iagaru et al., 2026) and an SVD diagnostic (Bhadra et al., 2021) into a per-instance object --- and where Gottschling et al.'s (2025) kernel constructions establish destabilizing existence, ours are cross-class, record-tight, and displayed. \textbf{Second}, we give a ground-truth-free, reconstructor-agnostic \textbf{test-time certificate} that contracts each measured mode by \textit{exactly} $\lambda/(\lambda+\sigma_i^2)$ --- a closed-form modal spectrum, not a feasible-set-diameter bound (Iagaru et al., 2026), a soft penalty (NPN; GSNR), a design-time predictability curve (GSNR), or a calibrated statistical interval (Angelopoulos et al., 2022) --- auditing BP, Tikhonov, CS-TV, and learned reconstructors uniformly --- including, in §8.4, three \textit{published} DL-GI pipelines decomposed on their own released or measured operators --- and \textbf{separating image quality from measurement accountability}. \textbf{Third}, that separation is demonstrated as an explicit protocol (perceptual/PSNR quality essentially unchanged while measurement residuals fall orders of magnitude), an axis none of the reconstruction or UQ lines report. \textbf{Fourth}, we convert null-space injection from an all-or-nothing fill into a single-scalar, pre-registered \textbf{honesty dial} that fuses VQAE structure and VQGAN detail and keeps $A\hat{x}_B = y$ \textit{exact} for every $B$ behind a locked gate --- moving the field from \textit{assessment} (does a reconstruction hallucinate?) to \textit{governance} (how much invented detail do we add, where the bucket is provably blind, without ever breaking or laundering the measurement?). We explicitly do \textbf{not} claim SOTA, do not beat diffusion, and never claim the measurement certifies invented texture --- the point is that it provably cannot. That inversion --- \textbf{reconstruct, becomes certify / bound / meter} --- is the slot this paper occupies.

\medskip\hrule\medskip

\section*{11. Discussion, Scope, and Limitations}
...
\subsection*{11.1 Simulation-only, single measurement problem}
...
\subsection*{11.2 The locked achievability claim is one operating point}
...
\subsection*{11.5 The failure detector is weak, and the certificate does not localize error}
